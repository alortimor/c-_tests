...
int main () {
  ...
  // if a parameter is to be passed by reference to a thread, then std::ref has to be used
  
  // std::thread t1( (Fctor()), std::ref(s) );
  
  //so now there exists a situation where the main thread and the t1 thread are sharing
  // the same memory, which is the string s. 
  // this may however create a data race problem, so moving can be used instead
