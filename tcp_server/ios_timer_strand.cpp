...
void better_late_than_never(const boost::system::error_code & ec
  ...
  if(ec) {
    ...
    tmr->async_wait(strand->wrap(boost::bind(&better_late_than_never, _1, tmr, strand)) );
...
int main(void) {
  ...
  boost::shared_ptr<boost::asio::io_service::strand> strand(new boost::asio::io_service::strand(*ios));
  ...
  timer->async_wait(strand->wrap(boost::bind(&better_late_than_never, _1, timer, strand)));
