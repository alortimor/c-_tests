...
private:
  void do_read() {
    ...
    sk.async_read_some(boost::asio::buffer(receive_data, max_length),
        [this](const boost::system::error_code & ec, std::size_t bytes_transferred) {
          if (!ec) {
            excep_log( "Before str assignment " + std::to_string(bytes_transferred));
            std::string str;
            str = receive_data;
...
/*
    boost::asio::async_read(sk, boost::asio::buffer(receive_data, max_length),
    ...
    boost::asio::async_read(sk, receive_data, '\n',
        [this](const boost::system::error_code& ec, std::size_t bytes_transferred) {
              if (!ec) {
  ...
  */
  ...
  void do_write(std::size_t length, int rows) {
    ...
    excep_log( std::string("Before write ") + send_data);
