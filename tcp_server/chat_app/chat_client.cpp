...
typedef boost::shared_ptr< queue<string_ptr> > message_q_ptr;

io_service service;
message_q_ptr message_q(new queue<string_ptr>);
...
int main(int argc, char** argv) {
	try 	{
		...
		std::cout<< "Welcome to the ChatServer\nType \"exit\" to quit" << endl;
...
void displayLoop(socket_ptr sock) {
	for(;;) {
		if(!message_q->empty()) {
			if(!isOwnMessage(message_q->front()))
				std::cout << "\n" + *(message_q->front());
...
bool isOwnMessage(string_ptr message) {
	return (message->find(*promptCpy) != string::npos);
}
